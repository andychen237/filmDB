...
Also, there were issues found using the .txt files on Mac, but changing the extension to .csv and removing '\r' from LINES TERMINATED BY fixed this problem. 
Otherwise, the .txt files should work fine on Windows, and both instances have been tested.